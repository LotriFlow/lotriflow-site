...
struct ContentView: View {
    ...
    private var logButton: some View {
        let readyColor = Color(red: 0.18, green: 0.75, blue: 0.55)
        let earlyColor = Color(red: 0.86, green: 0.32, blue: 0.38)
        let currentColor = model.isEarly ? earlyColor : readyColor
        let gradient = LinearGradient(colors: [
            currentColor.opacity(0.85),
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                .stroke(currentColor.opacity(0.65), lineWidth: 1.2)
        )
        .shadow(color: currentColor.opacity(0.25), radius: 6, x: 0, y: 3)
